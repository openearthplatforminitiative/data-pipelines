...
def forecast(context: AssetExecutionContext, dask_resource: DaskResource):
    ...
    processed_discharge_filepath = os.path.join(DBUTILS_PREFIX, S3_GLOFAS_FILTERED_PATH, formatted_date, 'filtered-*.parquet')
    ...
    forecast_df['valid_for'] = (forecast_df['valid_time'] - pd.Timedelta(days=1)).dt.date
    ...
    threshold_cols = [f"threshold_{int(threshold)}y" for threshold in GLOFAS_RET_PRD_THRESH_VALS]
    ...
    joined_ddf = dd.merge(forecast_df, threshold_df, on=['latitude', 'longitude'], how="left")
    ...
    for threshold, col_name in zip(GLOFAS_RET_PRD_THRESH_VALS, threshold_cols):
        ...
        joined_ddf[exceed_col] = (joined_ddf['dis24'] >= joined_ddf[col_name]).astype('int64')

        
    q1_fun = dd.Aggregation(
        name="q1",
        chunk=lambda s: s.quantile(0.25),
        agg=lambda s0: s0.quantile(0.25)
    )
    ...
    q3_fun = dd.Aggregation(
        name="q3",
        chunk=lambda s: s.quantile(0.75),
        agg=lambda s0: s0.quantile(0.75)
    )

    res = joined_ddf.groupby(['latitude', 'longitude', 'issued_on', 'valid_for', 'step']).agg(
        min_dis24=('dis24', 'min'),
        ...
        ).reset_index().drop(columns=['numeric_only'])
    ...
    control_df = res[(res['step'] == 1)][['latitude', 'longitude', 'median_dis24']].rename(columns={'median_dis24': 'control_dis'})

    # merge control_df with res on latitude and longitude
    detailed_forecast_df = dd.merge(res, control_df, on=['latitude', 'longitude'], how='left')


    ##########################################################################################

    df_for_timing = detailed_forecast_df.drop(columns=['min_dis24', 'q1_dis24', 'q3_dis24', 'max_dis24', 'control_dis'])
    ...
    filtered_ddf = df_for_timing[(df_for_timing['step'] >= 1) & (df_for_timing['step'] <= 10)] # change to 10

    # 2. Compute the maximum flood probability above the 2-year return period threshold for the first ten days
    max_ddf = filtered_ddf.groupby(['latitude', 'longitude']).agg({'p_above_2y': 'max'}).rename(columns={'p_above_2y': 'max_2y_start'}).reset_index()
    ...
    def condition_func(df):
        df['condition'] = np.where(df['p_above_20y'] >= 0.3, 4,
                                    np.where(df['p_above_5y'] >= 0.3, 3,
                                            np.where(df['p_above_2y'] >= 0.3, 2, 1)))
        return df
    ...
    def sort_and_select_first_row(df):
        sorted_df = df.sort_values(['latitude', 'longitude', 'condition', 'median_dis24'], ascending=False)
        first_row_df = sorted_df.groupby(['latitude', 'longitude']).first().reset_index()
        return first_row_df

    ddf_maps = ddf_conds.map_partitions(sort_and_select_first_row, meta=ddf_conds).drop(columns=['median_dis24'])
    ...
    def peak_timing_func(df):
        df['peak_timing'] = np.where((df['peak_step'].isin(range(1, 10))) & (df['max_2y_start'] >= 0.30), 
                                    GLOFAS_FLOOD_PEAK_TIMINGS['black_border'],
                                    np.where((df['peak_step'] > 10) & (df['max_2y_start'] < 0.30), 
                                            GLOFAS_FLOOD_PEAK_TIMINGS['grayed_color'], 
                                            GLOFAS_FLOOD_PEAK_TIMINGS['gray_border']))
        return df


    ddf = ddf_maps.map_partitions(peak_timing_func).drop(columns=['condition', 'max_2y_start'])
    ...
    df_for_tendency = detailed_forecast_df.drop(columns=['q1_dis24', 'q3_dis24', 'p_above_2y', 'p_above_5y', 'p_above_20y'])

    grid_cell_tendency = (
    df_for_tendency.groupby(['latitude', 'longitude']).agg(
    ...
    def tendency_func(df):
        df['tendency'] = np.where(df['max_median_dis'] > df['control_dis'] * 1.10, 
                                GLOFAS_FLOOD_TENDENCIES['increasing'],
                                np.where((df['min_median_dis'] <= df['control_dis'] * 0.90) &
    ...
    df_for_intensity = detailed_forecast_df.drop(columns=['min_dis24', 'q1_dis24', 'q3_dis24', 'max_dis24', 'control_dis'])
    ...
    def intensity_func(df):
        df['intensity'] = np.where(df['max_p_above_20y'] >= 0.30, 
                                    GLOFAS_FLOOD_INTENSITIES['purple'],
                                    np.where(df['max_p_above_5y'] >= 0.30,
                                            GLOFAS_FLOOD_INTENSITIES['red'],
                                            np.where(df['max_p_above_2y'] >= 0.30,
    ...
    final_df = dd.merge(ddf, grid_cell_tendency, on=['latitude', 'longitude'], how='left')
    summary_forecast_df = dd.merge(final_df, grid_cell_intensity, on=['latitude', 'longitude'], how='left')
    ...
    summary_forecast_df.to_parquet(summary_forecast_file_path, write_index=False, overwrite=True)
    detailed_forecast_df.to_parquet(detailed_forecast_file_path, write_index=False, overwrite=True)
