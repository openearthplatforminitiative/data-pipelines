...
def preprocess_reproject(context: AssetExecutionContext, dask_resource: DaskResource, raw_imagery: dict) -> list:
    virts = []

    with tqdm(desc="Reprojecting image tiles",
              total=len(raw_imagery),
              unit="tile") as progress:
        for id in raw_imagery:
            product = raw_imagery[id]
...
def preprocess_optimize(context: AssetExecutionContext):
    ...
    with tqdm(desc="Tile optimizing",
              total=len(dirlist),
              unit="tile") as progress:
        for file in dirlist:
